# Hi 👋, I'm André Lopes

🚀 **Aspiring Developer | Student & Creator** 🇧🇷 Indaiatuba, Brazil

I’m currently focused on **learning, building, and exploring** the world of software development. 
My goal is to transition into professional development through study and hands-on projects.

I enjoy working with **game development, Python, C#, and content creation.**

---

## 🌐 Where to find me

<p align="left">
  <a href="https://github.com/SEU_USUARIO_AQUI">
    <img src="https://img.shields.io/badge/GitHub-ANDRE_LOPES-181717?style=for-the-badge&logo=github" />
  </a>
  <a href="https://www.youtube.com/@SEU_CANAL">
    <img src="https://img.shields.io/badge/YouTube-CONTENT_CREATION-FF0000?style=for-the-badge&logo=youtube&logoColor=white" />
  </a>
</p>

---

## 🧠 What I'm working on

- 🎮 **Game Dev:** Building a 3D Rubik's Cube simulator using Python & Ursina.
- 🐍 **Languages:** Strengthening my foundation in Python and C#.
- 🎥 **Content:** Studying YouTube content creation & digital media.
- 🧩 **Problem Solving:** Passionate about chess, logic puzzles, and speedcubing.
- 🎓 **Education:** Always learning and preparing for new challenges in software engineering.

---

## 🛠️ My Current Toolkit

### Languages & Frameworks
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![C#](https://img.shields.io/badge/C%23-512BD4?style=for-the-badge&logo=csharp)
![Unity](https://img.shields.io/badge/Unity-000000?style=for-the-badge&logo=unity)

### Tools
![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git)
![Visual Studio Code](https://img.shields.io/badge/VSCode-007ACC?style=for-the-badge&logo=visual-studio-code)
![Figma](https://img.shields.io/badge/Figma-F24E1E?style=for-the-badge&logo=figma)

---

## 📊 GitHub Stats

<p align="left">
  <img height="170" src="https://github-readme-stats.vercel.app/api?username=Andre-wellington&show_icons=true&theme=tokyonight&count_private=true" />
  
  <img height="170" src="https://github-readme-streak-stats.herokuapp.com/?user=Andre-wellington&theme=tokyonight" />

  <img height="170" src="https://github-readme-stats.vercel.app/api/top-langs/?username=Andre-wellington&layout=compact&theme=tokyonight" />
</p>

---

## 🚀 Philosophy

> _"Every line of code is a step toward understanding how the world works. 
> I'm here to build, learn, and share the process."_

---
